...
class aspect_Barista(nn.Module):
    
    def __init__(self, args, vecs, pdataset,
                 src_syn1, src_syn2, src_neg,
                 trg_syn1, trg_syn2, trg_neg,
                 output_dim=5):
        super(aspect_Barista, self).__init__()
    ...
    def project(self, x, y):
        ...
        """
        x_lookup = torch.LongTensor(np.array([self.w2idx[w] for w in x]))
        y_lookup = torch.LongTensor(np.array([self.w2idx[w] for w in y]))
        x_embedd = self.emb(Variable(x_lookup))
        y_embedd = self.emb(Variable(y_lookup))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-se', '--embeddings', default='embeddings/barista/sg-300-window4-negative20_en_es.txt')
    ...
    aBarista = aspect_Barista(args, vecs, pdataset,
    ...
    print('Training...')
    try:
        aBarista.fit(train_data, dev_data,
                 trg_dev,
                 epochs=args.epochs)
    ...
    src_pred = aBarista.predict(l, r, t).data.numpy().argmax(1)
    ...
    trg_pred = aBarista.predict(trgl, trgr, trgt).data.numpy().argmax(1)
    ...
    outfile = os.path.join('predictions', args.src_dataset, '{0}-{1}'.format(args.src_lang, args.trg_lang),
                            'aBarista-binary:{0}_epochs:{1}_learningrate:{2}_weightdecay:{3}_batchsize:{4}.txt'.format(
    ...
    plotfile=os.path.join('figures', args.src_dataset, '{0}-{1}'.format(args.src_lang, args.trg_lang),
                            'aBarista-binary:{0}_epochs:{1}_learningrate:{2}_weightdecay:{3}_batchsize:{4}.pdf'.format(
                                args.binary, args.epochs, args.learning_rate, args.weight_decay, args.batch_size))
    os.makedirs(os.path.dirname(plotfile), exist_ok=True)
    aBarista.plot(outfile=plotfile)

    cross_dev_f1 = aBarista.history['cross_f1'][-1]
