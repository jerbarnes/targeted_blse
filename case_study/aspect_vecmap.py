...
class aspect_VecMap(nn.Module):
    
    def __init__(self, args, src_vecs, trg_vecs, pdataset,
                 output_dim=5):
        super(aspect_VecMap, self).__init__()
        ...
        # project based bilingual lexicon (pdataset)
        W = self.get_W(pdataset, src_vecs, trg_vecs)
    ...
    def get_W(self, pdataset, src_vecs, trg_vecs):
        X, Y = [], []
...
if __name__ == '__main__':
    ...
    parser.add_argument('-tl', '--trg_lang', default='eu')
    parser.add_argument('-se', '--src_embeddings', default='embeddings/sg-300-en.txt')
    parser.add_argument('-te', '--trg_embeddings', default='embeddings/sg-300-eu.txt')
    parser.add_argument('-sd', '--src_dataset', default='datasets')
    parser.add_argument('-e', '--epochs', default=300, type=int)
    parser.add_argument('-a', '--alpha', default=0.3, type=float)
    ...
    aVec = aspect_VecMap(args, src_vecs, trg_vecs, pdataset,
                         output_dim=output_dim)

    print('Training...')
    try:
        aVec.fit(train_data, dev_data,
                 epochs=args.epochs)
    ...
    src_pred = aVec.predict(l, r, t, src=True).data.numpy().argmax(1)
    ...
    trg_pred = aVec.predict(trgl, trgr, trgt, src=False).data.numpy().argmax(1)
    cross_f1 = per_class_f1(trgy, trg_pred)
    print(cross_f1)
    print(cross_f1.mean())

    outfile = os.path.join('predictions', '{0}-{1}'.format(args.src_lang, args.trg_lang),
                            'aVecMap-binary:{0}_epochs:{1}_learningrate:{2}_weightdecay:{3}_batchsize:{4}-embeddings:{5}.txt'.format(
                                args.binary, args.epochs, args.learning_rate, args.weight_decay, args.batch_size, args.trg_embeddings))
    print_prediction(trg_pred, outfile)

    goldfile = os.path.join('predictions', '{0}-{1}'.format(args.src_lang, args.trg_lang), 'binary:{0}-gold.txt'.format(args.binary))
    print_prediction(trgy, goldfile)


    with open(os.path.join('predictions', '{0}-{1}'.format(args.src_lang, args.trg_lang), 'binary:{0}-text.txt'.format(args.binary)), 'w') as out:
    ...
    #                        'aVecMap-binary:{0}_epochs:{1}_learningrate:{2}_weightdecay:{3}_batchsize:{4}.pdf'.format(
